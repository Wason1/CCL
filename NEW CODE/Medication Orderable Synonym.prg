...
;	, synonym_cki_count = count(distinct ocs.synonym_id) over (partition by ocs.cki)	
	...
	, synonym_orders = if(o_count.count >"0") o_count.count	
	else "0"	
	endif	
...
from		
	...
	, (left join (select o.synonym_id, count = count(*) from orders o group by o.synonym_id) o_count on o_count.synonym_id = ocs.synonym_id)	
...
;and	cnvtupper (oc.primary_mnemonic) < "E*"	; use these if the audit fails in any domain
