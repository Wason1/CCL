...
;	, cat_concept_cki = oc.concept_cki	
	...
	, primary_synonym_last_updater = if(ocs.synonym_id > 0 and ocs.updt_id = 0) "0"	
	else p_ocs.name_full_formatted	
	endif	
	, orders_for_all_synonyms_under_primary = if(o_count.count > "0") o_count.count	
	else "0"	
	endif	
	, results_for_all_synonyms_under_primary = if(res_count.count > "0") res_count.count	
	else "0"	
	endif	
	...
	, alias_inbound_last_updater = if(cva_oc.code_value > 0 and cva_oc.updt_id = 0) "0"	
	...
	, alias_outbound_last_updater = if(cvo_oc.code_value > 0 and cvo_oc.updt_id = 0) "0"	
	...
	, task_mapping_last_updater = if(otxr.reference_task_id > 0 and otxr.updt_id = 0) "0"	
	...
	, event_code_mapping_last_updater = if(cver.event_cd > 0 and cver.updt_id = 0) "0"	
...
;	endif	
	...
	, dup_check_last_updater = if(dcheck.catalog_cd > 0 and dcheck.updt_id = 0) "0"	
...
;	4,"Yes (Auto verify regardless of alerts)")	
	...
	, primary_settings_last_updater = if(oc.catalog_cd > 0 and oc.updt_id = 0) "0"	
...
;	, (left join mltm_drug_name mltmdn on concat("MUL.ORD-SYN!",trim(cnvtstring(mltmdn.drug_synonym_id))) = ocs.cki)	
	...
	, (left join (select o.catalog_cd, count = count(*) from orders o group by o.catalog_cd) o_count on o_count.catalog_cd = oc.catalog_cd)	
	, (left join (select ce.catalog_cd, count = count(*) from clinical_event ce where ce.view_level = 1 group by ce.catalog_cd) res_count on res_count.catalog_cd = oc.catalog_cd)	
