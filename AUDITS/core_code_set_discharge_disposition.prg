select	; 'Discharge Disposition' code set alignment	
	cv.code_set	
	...
	, encntrs_using = if(encntrs.count > "0") encntrs.count	
	else "0"	
	endif	
	, encntr_flexes_using = if(encntr_flexes.count > "0") encntr_flexes.count	
	else "0"	
	endif	
;	, encntr_locs_using = if(encntr_locs.count > "0") encntr_locs.count	
;	else "0"	
;	endif	
	, pm_trans_old_using = if(pm_trans_old.count > "0") pm_trans_old.count	
	else "0"	
	endif	
	, pm_trans_new_using = if(pm_trans_new.count > "0") pm_trans_new.count	
	else "0"	
	endif	
	...
	, VEMD = cve_VEMD.field_value	
	, VEMD_last_update = format(cve_VEMD.updt_dt_tm, "dd/mm/yyyy hh:mm:ss")	
	, VEMD_last_updater = if(cve_VEMD.code_value > 0 and p_cve_VEMD.name_full_formatted = null) cnvtstring(cve_VEMD.updt_id)	
	else p_cve_VEMD.name_full_formatted	
	endif	
...
from		
	...
	, (left join code_value_extension cve_VEMD on cve_VEMD.code_value = cv.code_value	; Code value extension tab in Corecodebuilder
	and cve_VEMD.field_name = "VEMD"	
	)	
	, (left join prsnl p_cve_VEMD on p_cve_VEMD.person_id = cve_VEMD.updt_id)	
	, (left join (select e.disch_disposition_cd, count = count(*) from encounter e group by e.disch_disposition_cd) encntrs on encntrs.disch_disposition_cd = cv.code_value)	
	, (left join (select e_flex.disch_disposition_cd, count = count(*) from encntr_flex_hist e_flex group by e_flex.disch_disposition_cd) encntr_flexes on encntr_flexes.disch_disposition_cd = cv.code_value)	
;	, (left join (select e_loc.disch_disposition_cd, count = count(*) from encntr_loc_hist e_loc group by e_loc.disch_disposition_cd) encntr_locs on encntr_locs.disch_disposition_cd = cv.code_value)	
	, (left join (select pm_t.o_disch_disp_cd, count = count(*) from pm_transaction pm_t group by pm_t.o_disch_disp_cd) pm_trans_old on pm_trans_old.o_disch_disp_cd = cv.code_value)	
	, (left join (select pm_t.n_disch_disp_cd, count = count(*) from pm_transaction pm_t group by pm_t.n_disch_disp_cd) pm_trans_new on pm_trans_new.n_disch_disp_cd = cv.code_value)	
		
plan	cv	
where	cv.code_set = 19	; 'DISCH_DISPOS' code set
join	p_cv_ds	
join	p_cv	
join	cvs	
join	cva	
