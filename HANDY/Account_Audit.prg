select	; WHS users	that were created in the last 8 hours by the current user
	...
	, p.person_id	
	, last_logon = format( p_activity.last_act , "dd/mm/yyyyy")	
...
;	else ""	
;	endif	
	, org_dr_alias_beg_dt = format(p_a_org_dr.beg_effective_dt_tm, "dd/mm/yyyy")	
	, org_dr_alias_end_dt = format(p_a_org_dr.end_effective_dt_tm, "dd/mm/yyyy")	
...
where	p.person_id > 0	; ignore '0' database row
		...
		and
		p.updt_dt_tm > cnvtlookbehind("8, h"); filters for accounts changed in the last 8 hours
;and	p.active_ind = 1	
...
and	p.position_cd not in (	
	0	; ignore blank position (‘non-user' EMR accounts, usually GPs)
